<template>
    <div>
        ...
        <Collapse
          v-model="value1"
          draggable
          @dragComplete="handleDragComplete">
            <Panel
              :key="item.name"
              :name="item.name"
...
export default {
    ...
    methods: {
      handleDragComplete (dragIndex, dropIndex) {
        const dataCopy = this.collapseData.slice()
        // 对数据进行交换
        dataCopy.splice(dropIndex, 0, ...dataCopy.splice(dragIndex, 1))
        this.collapseData = []
        this.collapseData.push(...dataCopy)
      }
    }
}
</script>
